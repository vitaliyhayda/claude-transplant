...
The menubar shows two columns of email over plan, with FROM on the left and TO on the right. When the active identity is known, TO starts on the most recently used account other than the signed-in one, while every other account starts selected as a source. When it is unknown, choose TO explicitly. An explicit destination choice is preserved across refreshes. Uncheck only the accounts to leave behind. Curves converge on TO, chevrons show direction, destination changes swap lane identity, and source changes fade only that lane. Long inventories scroll together. The active account is tagged, pending sources are tagged, and Finish pending continues the same move. A held local source and its dependent forks can finish under any login. The app quietly retries held local work when its workers stop and pending cloud work when its source account becomes active, with a 60-second cloud retry backoff. A cheap process check skips repeated transcript inventories while held workers remain. Read-only account refreshes stop after 60 seconds if they fail to return. Mutating sweeps keep their journal and HTTP timeouts, and errors remain visible in the panel. Keep local cancels held work and pending cloud checks without reversing completed moves. It starts at login, shows progress in the icon, and posts a notification when done. The app bundles its own CLI, so run the menubar command again after upgrading. Use `menubar --snapshot panel.png` to render the live panel without installing it, or use `menubar --remove` to uninstall. The animation above is rendered from the same Swift panel. It follows one person rotating between their own work and home logins. The home Personal account is the destination, Acme is active, and Northwind is unchecked before moving the 318 selected records, matching the terminal example.
...
- A later September 5 test of the installed status-item popup failed the v4 acceptance check. After a graceful shutdown, re-inventory of a 177-record archive exhausted the move deadline before the held record moved. Desktop reopened after about 30 seconds, the held source remained, and the 165 records moved earlier were still present. This blocks release until the restart path is corrected.
...
A full installed-popup test subsequently reproduced two UI gaps alongside that timeout. Finish pending repeated connected Remote Control refusals, and the restart warning represented two sessions as four unidentified helper/child processes. Native automation did not expose the windowless status popup reliably, so the test used actual mouse clicks and screenshots on the external display. Snapshot renders and the isolated window harness had not exercised this path.

To regenerate the demo, compile `swiftc -O -parse-as-library -o /tmp/claude-transplant-demo menubar.swift`, then run `/tmp/claude-transplant-demo --demo /tmp/claude-transplant-frames`. Encode its PNGs using one shared 256-color palette and the durations in `durations.json`. The September 5 animation has ten frames at 1084 by 718 pixels, including the final result height in every frame. The account data matches the terminal example, and decoded GIF frames were checked for retained blue controls and green active tags.
