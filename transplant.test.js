...
test('move, rerun, undo across accounts', async () => {
  ...
  assert.deepEqual(inv.move.map((s) => s.id).sort(), [SOURCE, id(200), id(500), id(600), id(710), id(800)].sort())
  ...
  assert.equal(result.receipt.sessions.length, 4)
  ...
  assert.match(stages[0], /^fork 4 ✓ \| \d+ events \| 1 replay duplicates collapsed \| 2 failed$/)
  assert.equal(stages[1], 'sidecars 2 files | sha256 ✓')
  assert.equal(stages[2], 'desktop 4 records | 0 archived | 4 active')
  ...
  assert.equal((await readdir(h.dir('Z'))).length, 5)
  ...
  assert.equal(again.there.length, 5)

  const undone = await undo(h.paths)
  assert.ok(undone.dest)
  assert.equal((await readdir(h.dir('Z'))).length, 1)
  assert.equal((await readdir(undone.dest)).length, 10)
  assert.equal((await plan()).move.length, 6)
