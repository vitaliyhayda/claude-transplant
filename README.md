...
- The percentage replaces the arrow in a template image with the arrow's exact canvas size. A SwiftUI frame or overlay still widened the native menu item, so layout constraints alone were insufficient. Native checks measured the same 34-point item width for the idle symbol and percentages through 100%.
